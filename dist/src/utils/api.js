...
function factory() {
    ...
    async function resolveResponse(response) {
        try {
            ...
            if (response.status === 451)
                console.log(`Ocorreu um erro no servidor com o status code ${response.status}`);
            // captura algum erro e exibe o alert no app
            if (response.status >= 500)
                console.log(`Ocorreu um erro no servidor com o status code ${response.status}`);
            const contentType = response.headers.get('content-type');
            if (!response.ok)
                return fetchAdapterError(response);
    ...
    */
    return {
        ...
        */
        get: async (url, object = null, options) => {
        ...
        * @since v0.1 by [`Gabriel henrique`](https://github.com/gab-h3nrique)
        */
        post: async (url, object = null, options) => {
            // const user = await storage.get('user')
            // const loja = await storage.get('loja')
        ...
        * @since v0.1 by [`Gabriel henrique`](https://github.com/gab-h3nrique)
        */
        delete: async (url, object = null, options) => {
            // const user = await storage.get('user')
            // const loja = await storage.get('loja')
            const response = await fetch(baseUrl + url + `${object ? `?${new URLSearchParams(object)}` : ''}`, {
                method: 'DELETE',
        ...
        * @since v0.1 by [`Gabriel henrique`](https://github.com/gab-h3nrique)
        */
        upload: async (url, object = null, options) => {
            // const user = await storage.get('user')
            // const loja = await storage.get('loja')
...
* @since v0.1 by [`Gabriel henrique`](https://github.com/gab-h3nrique)
*/
const fetchAdapter = factory();
exports.default = fetchAdapter;
async function fetchAdapterError(response) {
