...
const brandType_1 = require("@/types/brandType");
const react_1 = __importStar(require("react"));
function BrandModal({ isOpen, brand, onClose }) {
    ...
    const [edited, setEdited] = (0, react_1.useState)(brandType_1.EMPTY_BRAND);
    async function remove() {
        try {
            if (saveLoading)
                return;
            setRemoveLoading(true);
            const { data, total, success, message, ...rest } = await http_1.default.delete('/api/auth/brands', { id: edited.id });
            if (!success)
    ...
    async function save() {
        try {
            if (saveLoading)
                return;
            setSaveLoading(true);
            const { data, total, success, message, ...rest } = await http_1.default.post('/api/auth/brands', edited);
            if (!success)
    ...
    (0, react_1.useEffect)(() => {
        if (brand)
            setEdited({ ...brand });
        if (!isOpen) {
            setEdited(brandType_1.EMPTY_BRAND);
        }
    }, [isOpen]);
    return ((0, jsx_runtime_1.jsx)("section", { className: `bg-background-1 dark:bg-background-1-dark w-full h-full ${isOpen ? 'flex' : 'hidden'}`, children: (0, jsx_runtime_1.jsx)("div", { className: 'py-12 gap-1 w-full h-full flex flex-col items-center relative', children: (0, jsx_runtime_1.jsxs)("div", { className: "flex flex-col gap-12 w-full max-w-[45rem] pb-10", children: [(0, jsx_runtime_1.jsx)("section", { onClick: () => console.log(brand, edited), className: 'flex flex-col', children: (0, jsx_runtime_1.jsx)(Texts_1.Paragraph, { children: "Marca" }) }), (0, jsx_runtime_1.jsx)("section", { className: 'grid grid-cols-4 gap-6', children: (0, jsx_runtime_1.jsxs)("div", { className: 'col-span-4 max-w-[20rem]', children: [(0, jsx_runtime_1.jsxs)(Texts_1.Description, { className: 'mb-1 text-color-2 dark:text-color-2-dark', children: ["Nome", (0, jsx_runtime_1.jsx)("span", { className: 'ml-1 text-red-500', children: "*" })] }), (0, jsx_runtime_1.jsx)(Input_1.default, { type: 'text', onChange: (e) => setEdited(prev => ({ ...prev, name: e.target.value })), value: edited?.name })] }) }), (0, jsx_runtime_1.jsx)(Hr_1.default, {}), (0, jsx_runtime_1.jsxs)("section", { className: 'flex gap-4', children: [(0, jsx_runtime_1.jsxs)(Button_1.default, { onClick: remove, className: `justify-center text-color-2 dark:text-color-2-dark min-w-[82px] ${edited.id && edited.id != -1 ? 'flex' : 'hidden'}`, children: [(0, jsx_runtime_1.jsx)(Svg_1.default.Spinner, { className: `w-5 h-5 fill-background-2 dark:fill-background-2-dark opacity-[.4] ${!removeLoading && 'hidden'}` }), !removeLoading ? 'Excluir' : ''] }), (0, jsx_runtime_1.jsx)(Button_1.default, { onClick: () => onClose(), className: 'ml-auto text-color-2 dark:text-color-2-dark', children: "Cancelar" }), (0, jsx_runtime_1.jsxs)(Button_1.default, { onClick: save, className: 'flex justify-center bg-primary text-background-2 dark:text-background-2-dark min-w-[82px]', children: [(0, jsx_runtime_1.jsx)(Svg_1.default.Spinner, { className: `w-5 h-5 fill-background-2 dark:fill-background-2-dark opacity-[.4] ${!saveLoading && 'hidden'}` }), !saveLoading ? 'Salvar' : ''] })] })] }) }) }));
}
exports.default = (0, react_1.memo)(BrandModal);
